class CustomersController < ApplicationController
  ...
  def index
    @customers = current_user.company.customers.includes(:user)
  end
  ...
  private
    ...
    def set_customer
      @customer = current_user.company.customers.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def customer_params
      params.expect(customer: [ :name, :phone, :email, :address, :user_id, :media_source_id ])
    end
end
